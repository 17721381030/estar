...
function randerReportUnit(blockId,report_list,report_detail,nodeData,nodeName,showTitle){
	if(nodeData && nodeData.length > 0){
		...
		for(var i = 0; i < nodeData.length; i++){
			...
			errorMsg += (i+1)+"、"+r.msg+"";
			trMsg += "<img class='img_format' style='padding-top:1px;' src='"+getImgUrl4Report(r.type)+"' />&nbsp;&nbsp;"+(i+1)+"、"+r.msg;
			
			if(blockId === BLOCK_NAME.LAW_ARTICLE_CHECK){
				trMsg += "<form id='law_"+i+"'>";
				trMsg += "<input type='hidden' name='rowId' value='"+r.rowId+"' />";
				trMsg += "<input type='hidden' name='d.id' value='"+r.rowId+"' />";
				trMsg += "<input type='hidden' name='d.tableType' value='"+r.tableType+"'/>";
				trMsg += "<input type='hidden' name='d.appraisalsStatus' />";
				trMsg += "<input type='hidden' name='d.appraisalsCause' />";
				if(r.appraisalsStatus === ""){
					trMsg += "&nbsp;&nbsp;&nbsp;&nbsp;<span class='statistics'>未评价</span><br/>";
				}else if(r.appraisalsStatus === "0"){
					trMsg += "&nbsp;&nbsp;&nbsp;&nbsp;<span class='statistics'>评价:确认瑕疵</span><br/>";
				}else if(r.appraisalsStatus === "1"){
					trMsg += "&nbsp;&nbsp;&nbsp;&nbsp;<span class='statistics'>评价:不是瑕疵."+"理由:"+r.appraisalsCause+"</span><br/>";
				}
				trMsg += "&nbsp;&nbsp;&nbsp;&nbsp;<span onclick='javascript:statistics("+i+",\"y\")' class='btn btn_statistics_y' >确认瑕疵</span><span onclick='javascript:statistics("+i+",\"n\")' class='btn btn_statistics_n' >不是瑕疵</span>";
				trMsg += "</form>";
			}
		}
...
function statistics(idx,bl){
	if(bl === "y"){
		$("#law_"+idx).find("input[name='d.appraisalsStatus']").val(0);
		submitStatistics("law_"+idx);
	}else if(bl === "n"){
		$("#law_"+idx).find("input[name='d.appraisalsStatus']").val(1);
		$("#saveAppraisalsCauseButton").attr("onclick","javascript:saveAppraisalsCause('law_"+ idx +"')");
		$("#appraisalsCauseDiv").show();
		/*$.dialog({
			title: "理由123456",
			content: "<textarea class='textarea' id='statistics_textarea'></textarea>",			
			width: 250,
			modal: true,
			ok: "确&nbsp;&nbsp;定",
			cancel: "取&nbsp;&nbsp;消",
			onOk: function() {
				$("#law_"+idx).find("input[name='d.appraisalsCause']").val($("#statistics_textarea").val());
				submitStatistics("law_"+idx);
			}
		});*/
	}
}
//201412291109 sengman
// content: "<textarea style='z-index:103' autofocus class='textarea' id='statistics_textarea'>123456</textarea>",


function submitStatistics(formId){
	var appraisalsStatus = $("#"+formId).find("input[name='d.appraisalsStatus']").val();
	var appraisalsCause = $("#"+formId).find("input[name='d.appraisalsCause']").val();
	$.ajax({
        url: sysPath+"/statistics_refreshXajDto.do",
        ...
        success: function(msg) {
        	if(appraisalsStatus === "0"){
	        	$("#"+formId).find(".statistics").html("评价:确认瑕疵");
        	}else{
	        	$("#"+formId).find(".statistics").html("评价:不是瑕疵.理由:"+appraisalsCause);
        	}
        }
	});
}
